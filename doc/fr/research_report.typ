...
- *Les propriétés de connexité*
  - Exemples :
    - Vérifie que $forall n [A]$, existe une arborescence couvrante dont la racine est $n [A]$
...
- *Les doublons de Node*
  - Définition : $forall n_1, n_2 in V_N^2$, $n_1, n_2$ sont des doublons SSI $n_1 {P_1} = n_2 {P_2}$, $forall p in P_1$, $n_1.p = n_2.p$ et $forall r$ tel que $r."src" = n_1 | n_2$ ou $r_1."dest" = n_1 | n_2$ serait un doublon si $n_1 = n_2$.
- *L’existence de Clés*
...
- *Dépendance Fonctionnelle (FD)*
  - Définition : $forall e in V_N union E_R$, avec $X, Y subset.eq e{P}$ et $L subset.eq e[L_e]$ et on définit par $(L, X -> Y)$ une FD sur $e$.
...
- *Dépendance d’un pattern de Graph (GFD)*
  - Définition : $forall e in V_N union E_R$, avec $X, Y subset.eq e{P}$, $L subset.eq e[L_e]$ et $G_p$ un pattern de graphe permettant d’appliquer la dépendance fonctionnelle au sous graphe $G' (V_N', E_R')$ induit de $G_p$ tel qu’on définit par $(L, G_p, X -> Y)$ une GFD sur $e$. Une autre approche nommé _gFD_ @Manouvrier2024PGFD consiste a exclure tous les nodes qui n'ont pas toutes les properties définit par la _FD_.
- *Validation par Requête personnalisable*
  - Intuition : L’approche de *GFD* est intéressante mais peut rapidement devenir complexe à étendre. Une approche de validation par requête complètement écrite par l’utilisateur permettrait plus de flexibilité. Bien que les *GFD* restent utile notamment pour normaliser la base de donnée.
#pagebreak()
...
- *Intervalles de données*
  - Définition : $forall e in V_N union E_R$, avec $X subset.eq e{P}$ et $I$ un ensemble de valeurs et $C$ une contrainte comme défini pour les CFD ; tel que on vérifie que $forall p in X$, $I$ contains $(e.p)$ AND $C = "true"$.
#pagebreak()
...
- *Validité du Property schema*
  - Exemple : Vérifie les propriétés d’intégrité _Uniqueness_, _Key_, _Existence_ et _property Type_ sont vérifiées.
