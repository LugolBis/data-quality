...
def weighted_hhi(
    ...
    :type group_by: str
    :param counted: Column to get the number of tracked pattern (like number of properties, etc...)
    :type counted: str
    ...
    return _weighted_score(
        df, score_label=hhi, group_by=group_by, population=population
    )
...
def nodes_degree(
    ...
    if df is None:
        ...
        pivot: pd.DataFrame = df_c.pivot_table(
            index="label", columns="degree", values="average", aggfunc="mean"
        ...
        df_c["CV"] = (
            (df_c["standard_deviation"])
            .div((df_c["average"]).replace(0, pd.NA))
            .fillna(2)
        )
...
def _weighted_score(
    ...
    """
    Compute a weighted score such as : **SUM(Score_label * Count_label) / SUM(Count_label)**

    :param df: The DataFrame who contains the `group_by` and `population` columns.
    :type df: pd.DataFrame
    :param score_label: The score computed for each label (which name is in the `group_by` column).
    :type score_label: pd.Series[Any]
    :param group_by: The column used to aggregate the computed score of `score_label`.
    :type group_by: str
    :param population: The column who contains the weight of each value of `score_label`.
    :type population: str
